...
### 0.9 同步上游 0.2.7（本 fork 版本随之升 0.2.8）

上游 `eb87d25` 替换了 NAI 5-full 的默认负面词：官方基线之后原串了一大段社区负面层（禁 chibi、大胸、暗调、高对比、具体画师名等题材/风格/身材偏好，原作者已声明弃用），与正向词和画师串长期打架；改为只排画质/解剖/水印等**缺陷**的精简层，其余模型维持官方基线。上游另补一条回归锁（禁题材词不得回潮）与 manifest 缓存戳修正（`b1e3ed3`）。

本 fork 的 `src/backends/nai.ts` 与上游基线零分叉，两个源码文件直接摘取（dist 由本 fork 自行重打，不取上游产物）；因上游已占用 0.2.7 版本号，fork 版本升为 **0.2.8**（`sync-version` 同步 manifest 缓存戳）。更新源仍指向本仓库，行为不变。

### 0.10 第七轮实跑回归（v0.2.9）

在「联姻妻子」卡上用 tavern-tanuki MCP 连续跑了 5 层楼（现代轻喜剧、1~2 张/楼），逐图核对 0.2.8 的第六轮规则。先记验收：人数标准词、`size` 不泄漏、女方表情/视线称谓前缀、物品互动「通用动词 + 物品」出路、0~3 张质量优先——全部零复发；`/send + /trigger` 自动化路径与泉此方 advice 归一化完全兼容。

本轮新暴露四类（全部 SFW 场景，前六轮的 NSFW 回归没覆盖到）：

1. **无面男覆盖块在 SFW 双人图里被压住**（4/4 张）：`faceless male on <锚点>`（on 绑定）+ 男性照带 `smile` / `looking at another`。「优先于以上所有条目」的覆盖块照样输给思维链通篇的「人人要有表情视线」。改法：协议与覆盖块两处加 token 级三连——faceless male 一律裸写不接 on；他与表情词、视线词**互斥**（同现即自相矛盾）；落笔前最后核对「每个 fm 都裸写、他名下没有任何表情词与视线词、锚点与服装齐全」。
2. **同色瞳色每人一份**（4/4 张）：`black eyes` 写了两遍。「每个角色的硬特征（发色/瞳色/体型）并列写出」与「共有特征只写一次」两条打架，模型选了并列。改法：明确**发色必须每人各写（锚点）**，**瞳色相同只写一个**、不同才各写；第三层自查加「共有瞳色只写了一份」。
3. **同框个人动作漏前缀**（1 次）：`wiping mouth with sleeve` 裸写（是她在擦嘴）。规范的动作绑定条款被理解成只管「waving/eating dango」这类典型动作，生活小动作（擦嘴、递纸）漏网。改法：明确「同框时任何个人动作都不许裸写——擦嘴、递东西、翻纸也算」，第三层加「每条个人动作 tag 都以称谓前缀开头」。
4. **单人男性图缺表情视线**（1 次）：厨房做饭图整张没有表情与视线——无面男开关让模型对「男性 + 表情」产生了过度泛化回避。改法：覆盖块与协议补「画面里没有女性时照常画脸，表情与视线照常必填，不因本条省略」。
