...
export async function buildAutoTagMessages(
  ...
  const newCharacterRule = `
   ...
   - **field:"new" 只发给库里确实没有的角色**：输出前先在【角色固定外貌库】区块里逐字搜一遍名字——同名条目已经存在，就按已建档处理、照抄库中字段，绝不再输出 field:"new"；重复建档会被插件丢弃，但凭印象重写的外貌已经落进了画面，瞳色发色从此漂移。觉得库字段与正文对不上时以库为准：衣着等差异按临时状态写进当张 tag，瞳色发色这类固定特征不许改写。
   ...
   - **字段值必须是 danbooru 画得出、模型认得的英文词**：身高体重等数字（178cm、50kg）、气质性格与身份评价（gentle handsome type、mature aura、professional cosplayer）一律不得写进任何字段——它们画不出来，只会占掉之后每张图的 tag 预算。sex 只写 1girl / 1boy——成年男性也是 1boy，1man、1woman 不是 danbooru 词，存进档案会被逐字照抄进之后每张图的人数位。
   - **临时状态一律不得写进任何字段**（档案会在他之后每一张图里被照抄）：动作、姿势、所在场景（lying on carpet、standing、sitting、unzipped、湿身、伤势等）；**临时发型**（盘发、扎发、披发、湿发、丸子头、编发、马尾等造型——hair 只写长度与发色两个信息，写 long silver hair；不要写 long silver hair in an elegant bun 这类带造型的值，也不要写 bun、updo 这类单独的造型值——造型哪怕看起来是长期发型也不入档，画图时按当张正文补）；以及**当前这身衣服**（cosplay、制服、礼服等只在某段剧情里穿的服饰）。把姿势写进去会让他之后每张都保持那个姿势，把盘发写进去会让他之后每张都盘着头发，把 cosplay 写进 outfit 会让他之后每张都穿着那身。
