...
- **实跑回归修正·第五轮（v0.2.7）**：新正文（上海地铁 + 贝尔法斯特 cosplay，51 张图）逐图复核后修掉六类问题——`size` 槽位被当成 tag 词写回构图位（根因是思维链槽位前言「槽位值就是 tag 词」盖过了局部禁令，改为前言里显式豁免 `size`）；人数 tag 必须覆盖画面里所有可见人物且每个数量词只写一次（第三人同框写 `2boys 1girl`，不再是 `1boy 1girl, 1boy`）；锚点只能是正文给出的发色/瞳色，不得从服装颜色推（`yellow jacket` 不等于 `yellow hair`）、也不得给配角编发色；绑定只在两人及以上同框时使用，生理效果词、状态与接触动作词、单人画面的解剖词一律裸写（`panties aside` 这类词接给男方会被画成男方穿着内裤），动作词与部位不许粘成一条；同一件服装/饰品的措辞整楼逐字复用（裤袜区分连裤与过膝袜，局部件不替代整件）；不露脸的局部特写允许省略表情与视线。另把规范里会被照抄的具体举例（发色 + 单品）全部换成中性占位，并声明例子只是占位。
