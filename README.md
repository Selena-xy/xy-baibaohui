...
- **提示词规范与出图渠道解耦**：新增「生成提示词规范」开关，可在 NAI 渠道页把自动 tag 从 NAI 规范切到 ComfyUI 单串规范（邻接绑定 + 圆括号转义），专门适配「NAI 协议 + ComfyUI 底层」的第三方站（如 latent.moe / Anima）。
- **生成自然语言独立开关**：非 ComfyUI 后端选用 ComfyUI 规范时，可单独决定是否附带自然语言描述（Anima / Flux 类底模建议开启）。
- **无面男开关**：男女同框时男性不画脸（落 `faceless male`、不写表情与视线），把视觉焦点让给女性角色；男性单独出镜时照常画脸。默认关。
- **内置提示词调优**：ComfyUI 规范/思维链补显式 NSFW 解剖落点；开启自然语言时 tag 精简到 20～30 个；思维链 B 段拆出【一次性】角色；表情词表补 7 个稳定词；同人身份 tag 统一为带转义的全小写形态（`shorekeeper \(wuthering waves\)`）。
- **多轮实跑回归修正**：按真实聊天导出逐图核对，收敛了人数 tag、动作词照抄与绑定写法、锚点来源、建档字段、同人 tag 形态、`size` 词不进 tag 串等一批问题，并定下「人数只用 danbooru 标准词、on 只许接穿戴件与解剖部位、单人画面不许写 looking at another、规范里的举例一律是占位不许照抄」。逐轮结论见 [DESIGN.md](./DESIGN.md) §0.3～§0.10。
